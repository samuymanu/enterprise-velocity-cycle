...
export class AlertService {
  ...
  async checkStockLevels(): Promise<void> {
    ...
    try {
      ...
      logger.info(`📊 Checking stock levels for ${products.length} products`);
      ...
      logger.info(`✅ Stock level check completed in ${duration}ms. Created ${alertsCreated.length} alerts.`);
  ...
  /**
   * Analiza el stock de un producto específico
   */
  private async analyzeProductStock(product: any): Promise<string[]> {
    ...
    try {
      ...
      // 1. Verificar stock agotado (prioridad más alta)
      if (product.stock === 0 && !hasOutOfStockAlert) {
        ...
        // Resolver alerta de stock bajo si existía
        if (hasLowStockAlert) {
      ...
      // 2. Verificar stock bajo
        ...
        const alertId = await this.createAlert({
          ...
          message: `El producto ${product.sku} tiene stock bajo (${product.stock} unidades, mínimo: ${product.minStock})`,
      ...
      // 3. Verificar sobrestock
        ...
        const alertId = await this.createAlert({
          ...
          message: `El producto ${product.sku} tiene sobrestock (${product.stock} unidades, máximo: ${product.maxStock})`,
      ...
      logger.error(`❌ Error analyzing stock for product ${product.sku}:`, error);
    }
  ...
  /**
   * Actualiza métricas de rendimiento del sistema de alertas
   */
    ...
    try {
      ...
      if (duration > performanceThreshold) {
        await this.createAlert({
          ...
          message: `La verificación de stock tardó ${Math.round(duration / 1000)} segundos (umbral: ${performanceThreshold / 1000}s)`,
