...
async function debugProduct() {
  try {
    ...
    // Buscar todos los productos para comparar (incluyendo status)
    const allProducts = await prisma.product.findMany({
      select: {
        ...
        costPrice: true,
        salePrice: true,
        status: true // Agregar status para ver si están activos o inactivos
      }
    });
    
    console.log('\n📊 Todos los productos en BD:');
    allProducts.forEach(p => {
      console.log(`${p.sku}: ${p.name} - Stock: ${p.stock}, Precio: $${p.salePrice}, Status: ${p.status}`);
    });
